...
Thank you for taking the time to contribute to `hyperspec`!
...
```
git clone https://github.com/jcrist/hyperspec.git
cd hyperspec/
```
...
At this point you're all set up to start contributing to `hyperspec`!
...
You now have a "development" build of `hyperspec` installed. This means that you
can make changes to the `.py` files and test them without requiring a rebuild
of hyperspec's C extension. Edit away!
...
By default `hyperspec` is built in release mode, with optimizations enabled. To
...
You can run all [benchmarks](https://jcristharif.com/hyperspec/benchmarks.html) with the `bench` recipe, for example:
...
`hyperspec` has a code of conduct that must be followed by all contributors to
the project. You may read the code of conduct
[here](https://github.com/jcrist/hyperspec/blob/main/.github/CODE_OF_CONDUCT.md).
