...
class File(hyperspec.Struct, kw_only=True, omit_defaults=True, tag="file"):
    name: str
    created_by: str
    created_at: str
...
class Directory(hyperspec.Struct, kw_only=True, omit_defaults=True, tag="directory"):
    name: str
    created_by: str
    created_at: str
    updated_by: str | None = None
...
class Benchmark:
    ...
    def run(self, data: bytes) -> dict:
        if self.schema is not None:
            data = hyperspec.convert(data, self.schema)
...
def json_benchmarks():
    ...
    dec = hyperspec.json.Decoder(Directory)
    dec2 = hyperspec.json.Decoder()

    return [
        Benchmark("hyperspec structs", None, enc.encode, dec.decode, Directory),
        Benchmark("hyperspec", hyperspec.__version__, enc.encode, dec2.decode),
...
def msgpack_benchmarks():
    ...
    dec = hyperspec.msgpack.Decoder(Directory)
    dec2 = hyperspec.msgpack.Decoder()
