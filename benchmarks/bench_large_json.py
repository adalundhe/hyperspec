...
class Package(hyperspec.Struct, gc=False):
    build: str
    build_number: int
    depends: tuple[str, ...]
    md5: str
...
class RepoData(hyperspec.Struct, gc=False):
    repodata_version: int
    info: dict
    packages: dict[str, Package]
    removed: tuple[str, ...]

decode = hyperspec.json.Decoder(RepoData).decode
"""
...
def main():
    ...
    benchmarks = [
        ...
        ("hyperspec", "hyperspec", MSGSPEC),
        ("hyperspec structs", None, MSGSPEC_STRUCTS),
    ]
    ...
    bench_names.add("hyperspec structs")
    ...
    with tempfile.NamedTemporaryFile() as f:
        ...
        best_mem, best_time = results["hyperspec structs"]
