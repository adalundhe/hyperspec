"""
This benchmark compares the installed library size between hyperspec and pydantic
...
def main():
    hyperspec_version, hyperspec_size = get_latest_manylinux_wheel_size("hyperspec")
    ...
    data = [
        ("hyperspec", hyperspec_version, hyperspec_size),
        (
            "pydantic",
            pydantic_version,
            pydantic_size
    ...
    hyperspec_size = next(s for l, _, s in data if l == "hyperspec")

    columns = ("", "version", "size (MiB)", "vs. hyperspec")
    rows = [
        (
            f"**{lib}**",
            version,
            f"{size / (1024 * 1024):.2f}",
            f"{size / hyperspec_size:.2f}x",
        )
