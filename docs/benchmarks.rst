...
.. note::
    ...
    nonbiased benchmark. I wrote hyperspec, naturally whatever benchmark I
...
This benchmark covers the common case when working with ``hyperspec`` or other
...
.. code-block:: python
    ...
    class File(hyperspec.Struct, kw_only=True, tag="file"):
        name: str
        created_by: str
    ...
    class Directory(hyperspec.Struct, kw_only=True, tag="directory"):
        name: str
        created_by: str
        created_at: datetime.datetime
        updated_by: str | None = None
...
`here <https://github.com/jcrist/hyperspec/tree/main/benchmarks/bench_validation>`__.

.. raw:: html

    <div id="bench-validate" style="width:75%"></div>

In this benchmark ``hyperspec`` is ~6x faster than ``mashumaro``, ~10x faster
...
message decoding (as done by ``hyperspec``) rather than as a secondary step with
...
In contrast, libraries like ``hyperspec`` that validate during decoding have none
...
schemas and data. ``hyperspec``'s small library size, schema representation, and
...
``hyperspec`` includes its own high performance JSON library, which may be used
by itself as a replacement for the standard library's `json.dumps`/`json.loads`
functions. Here we compare hyperspec's JSON implementation against several other
...
The full benchmark source can be found
`here <https://github.com/jcrist/hyperspec/blob/main/benchmarks/bench_encodings.py>`__.

.. raw:: html

    <div id="bench-json" style="width:75%"></div>

In this case ``hyperspec structs`` (which measures ``hyperspec`` with
``hyperspec.Struct`` schemas pre-defined) is the fastest. When used without
schemas, ``hyperspec`` is on-par with ``orjson`` (the next fastest JSON library).

This shows that ``hyperspec`` is able to decode JSON faster when a schema is
provided. Due to a more efficient in memory representation, JSON decoding AND
schema validation with ``hyperspec`` than just JSON decoding alone.
...
Likewise, ``hyperspec`` includes its own high performance MessagePack_ library,
which may be used by itself without requiring usage of any of hyperspec's
validation machinery. Here we compare hyperspec's MessagePack implementation
...
As with the JSON benchmark above, ``hyperspec`` with a schema provided (``hyperspec
structs``) is faster than ``hyperspec`` with no schema. In both cases though
``hyperspec`` is measurably faster than other Python MessagePack libraries like
...
<https://github.com/jcrist/hyperspec/blob/main/benchmarks/bench_large_json.py>`__.
...
- ``hyperspec`` decoding without a schema is the second best option for both
  memory usage and speed. When decoding without a schema, ``hyperspec`` makes the
...
- ``orjson`` and ``simdjson`` use 6-9x more memory than ``hyperspec`` in this
...
Here we benchmark common `hyperspec.Struct` operations, comparing their
...
The full benchmark source can be found `here
<https://github.com/jcrist/hyperspec/blob/main/benchmarks/bench_structs.py>`__.
...
- For equality comparison, hyperspec Structs are roughly 4x to 30x faster than
  the alternatives.
- For order comparison, hyperspec Structs are roughly 5x to 60x faster than the
  alternatives.
...
`hyperspec.Struct` instances implement several optimizations for reducing garbage
...
The full benchmark source can be found `here
<https://github.com/jcrist/hyperspec/blob/main/benchmarks/bench_gc.py>`__.
...
- `hyperspec.Struct` instances have the same memory layout as a class with
...
- `hyperspec.Struct` instances with ``gc=False`` have the lowest memory usage
...
Here we compare the on-disk size of ``hyperspec`` and ``pydantic``, its closest
equivalent.

The full benchmark source can be found `here
<https://github.com/jcrist/hyperspec/blob/main/benchmarks/bench_library_size.py>`__.
...
.. raw:: html
    ...
    var results_valid = [
        {"label": "hyperspec", "encode": 0.00016727479400015, "decode": 0.0004222057979986857, "memory": 0.640625},
    ...
    var results_json = [
        {"label": "hyperspec structs", "encode": 0.00014051752349996606, "decode": 0.00036725287499939443},
        {"label": "hyperspec", "encode": 0.00018274705249996258, "decode": 0.00048175174399875685},
    ...
    var results_msgpack = [
        {"label": "hyperspec structs", "encode": 0.00011157811949942698, "decode": 0.000347989668000082},
        {"label": "hyperspec", "encode": 0.00012483930500002316, "decode": 0.000487175850001222},
...
.. _hyperspec: https://jcristharif.com/hyperspec/
