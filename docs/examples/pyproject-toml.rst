...
Here we define a hyperspec schema for parsing and validating a ``pyproject.toml``
...
<https://github.com/jcrist/hyperspec/tree/main/examples/pyproject-toml>`__.
...
- Whether strings like URLs or `dependency specifiers`_ are valid. Some of
  these could be handled using hyperspec's existing :doc:`../constraints` system,
  but not all of them.
- Mutually exclusive field restrictions (for example, you can't set both
  ``project.license.file`` and ``project.license.text``). ``hyperspec`` currently
