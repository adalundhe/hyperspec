...
``hyperspec`` provides a few utilities for generating `JSON Schema`_
specifications from hyperspec-compatible :doc:`types <supported-types>` and
:doc:`constraints <constraints>`.

- `hyperspec.json.schema`: generates a complete JSON Schema for a single type.
- `hyperspec.json.schema_components`: generates JSON schemas for multiple types,
...
.. code-block:: python
    ...
    schema = hyperspec.json.schema(list[Product])

    # Print out that schema as JSON
    print(hyperspec.json.encode(schema))
