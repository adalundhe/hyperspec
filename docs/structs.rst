...
Structs are defined by subclassing from `hyperspec.Struct` and annotating the
...
.. code-block:: python
    ...
    >>> from hyperspec import Struct, field
...
- A "dynamic" default value. Here a new default value is used for every
  ...
  ``default_factory`` argument of `hyperspec.field` (as in ``b`` above). This
...
- Decoding into a struct type (e.g. ``hyperspec.json.decode(..., type=MyStruct)``)
- Converting into a struct type (e.g. ``hyperspec.convert(..., type=MyStruct)``)

In these cases any `TypeError` or `ValueError` exceptions raised by this method
will be considered "user facing" and converted into a `hyperspec.ValidationError`
...
.. code-block:: python
    ...
    >>> class Interval(hyperspec.Struct):
    ...     low: float
    ...     high: float
    ...
    ...
    >>> hyperspec.json.decode(b'{"low": 2, "high": 1}', type=Interval)  # invalid interval from JSON
    Traceback (most recent call last):
      File "<stdin>", line 6, in __post_init__
    ValueError: `low` may not be greater than `high`
    ...
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: `low` may not be greater than `high`
...
.. code-block:: python
   ...
   >>> class Invalid(hyperspec.Struct):
   ...     a: str = ""
...
.. code-block:: python
   ...
   >>> class Example(hyperspec.Struct, kw_only=True):
   ...     a: str = ""
...
Like `dataclasses`, `hyperspec.Struct` types will exclude any attribute
...
`hyperspec.Struct` class are not checked at runtime during normal use. Types are
...
.. code-block:: python
    ...
    ... hyperspec.json.decode(b'{"x": 1.0, "y": "oops"}', type=Point)
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `float`, got `str` - at `$.y`

This is intentional. Static type checkers like mypy_/pyright_ work well with
``hyperspec``, and can be used to catch bugs without ever running your code. When
...
aren't known until runtime. As such, ``hyperspec`` does perform type validation
...
decode a message into a type validated `hyperspec.Struct` than into an untyped
`dict`.
...
If using Python 3.10+, `hyperspec.Struct` types can be used in `pattern matching`_
...
.. code-block:: python
    ...
    >>> class Point(hyperspec.Struct, order=True):
    ...     x: float
    ...     y: float
...
.. code-block:: python
    ...
    >>> class Point(hyperspec.Struct, eq=False):
    ...     x: float
    ...     y: float
...
.. code-block:: python
    ...
    >>> class Get(hyperspec.Struct):
    ...     key: str

    >>> msg = hyperspec.json.encode(Get("my key"))

    >>> msg  # No type information present in the message
    b'{"key":"my key"}'

    >>> hyperspec.json.decode(msg, type=Get)
    Get(key='my key')
...
``hyperspec`` handles this through the use of `Tagged Unions`_. A new field (the
...
.. code-block:: python
    ...
    ... class Get(hyperspec.Struct, tag=True):
    ...     key: str

    >>> class Put(hyperspec.Struct, tag=True):
    ...     key: str
    ...     val: str
    ...
    ... dec = hyperspec.json.Decoder(Union[Get, Put])
...
.. code-block:: python
    ...
    ... class TaggedBase(hyperspec.Struct, tag_field="op", tag=str.lower):
    ...     pass
...
By default, ``hyperspec`` encodes all fields in a Struct type, including optional
...
.. code-block:: python
    ...
    >>> hyperspec.json.encode(alice)  # default values are present in encoded message
...
.. code-block:: python
    ...
    >>> class User(hyperspec.Struct, omit_defaults=True):
    ...     name : str
    ...     email : Optional[str] = None
    ...     groups : Set[str] = set()
    ...
    >>> hyperspec.json.encode(alice)  # default values are omitted
    b'{"name":"alice"}'

    >>> bob = User("bob", email="bob@company.com")

    >>> hyperspec.json.encode(bob)
...
By default ``hyperspec`` will skip unknown fields encountered when decoding into
...
.. code-block:: python
    ...
    >>> class Example(hyperspec.Struct, forbid_unknown_fields=True):
    ...     field_one: int
    ...     field_two: bool = False
    ...
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Object contains unknown field `field_twoo`
...
``hyperspec`` supports two places for configuring a field's name used for
encoding/decoding:
...
use the ``name`` argument in `hyperspec.field`. Any fields declared with this
...
.. code-block:: python
    ...
    >>> class Example(hyperspec.Struct, rename="camel"):
    ...
    ... hyperspec.json.decode(b'{"fieldOne": 3, "fieldTwo": "four"}', type=Example)
    Example(field_one=3, field_two='four')

    >>> # Decoding errors also use the renamed field names
    ... hyperspec.json.decode(b'{"fieldOne": 5}', type=Example)
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Object missing required field `fieldTwo`
...
prefer to fully-uppercase these components (``FQDN``), but ``hyperspec`` has no
...
.. code-block:: python
    ...
    class V1PodSpec(hyperspec.Struct, rename=v1podspec_names):
        ...
...
`hyperspec.field` are used, names set explicitly via `hyperspec.field` take
precedence.
...
.. code-block:: python
    ...
    >>> class Example(hyperspec.Struct, rename="camel"):
    ...     field_x: int
    ...     field_y: int = hyperspec.field(name="y")  # set explicitly

    >>> hyperspec.json.encode(Example(1, 2))
    b'{"fieldX":1,"y":2}'
...
.. code-block:: python
    ...
    >>> hyperspec.json.encode(Point(1, 2))
    b'{"x":1,"y":2}'
...
.. code-block:: python
    ...
    >>> class Point2(hyperspec.Struct, array_like=True):
    ...     x: int
    ...     y: int

    >>> hyperspec.json.encode(Point2(1, 2))
    b'[1,2]'

    >>> hyperspec.json.decode(b'[3,4]', type=Point2)
    Point2(x=3, y=4)
...
.. code-block:: python
    ...
    >>> class Get(hyperspec.Struct, tag=True, array_like=True):
    ...     key: str

    >>> class Put(hyperspec.Struct, tag=True, array_like=True):
    ...     key: str
    ...     val: str

    >>> hyperspec.json.encode(Get("my key"))
    b'["Get","my key"]'
...
In some cases it can be useful to dynamically generate `hyperspec.Struct` classes
at runtime. This can be handled through the use of `hyperspec.defstruct`, which
has a signature similar to `dataclasses.make_dataclass`. See
`hyperspec.defstruct` for more information.

.. code-block:: python

    >>> import hyperspec

    >>> Point = hyperspec.defstruct("Point", [("x", float), ("y", float)])
...
You can define project-wide :class:`hyperspec.Struct` policies at class-creation
time by extending the :class:`hyperspec.StructMeta` metaclass.
...
You can also mix :class:`hyperspec.StructMeta` with other metaclasses. One common
...
Here :class:`hyperspec.Struct` participates fully in the ABC machinery:
...
.. important::
    ...
    - Classes with a :class:`hyperspec.StructMeta`-derived metaclass do not
      *technically* need to inherit from :class:`hyperspec.Struct`, but it is
      recommended to do so for static typing support in IDEs and other tools.
    - Mixing :class:`hyperspec.StructMeta` with arbitrary metaclasses
      is not supported. Only combinations involving :class:`abc.ABCMeta`
      (or its subclasses) are guaranteed to work. Prefer using
      :meth:`object.__init_subclass__` on a :class:`hyperspec.Struct` base class
...
By default, `hyperspec.Struct` types will only be tracked if they contain a
