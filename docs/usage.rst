...
``hyperspec`` supports multiple serialization protocols, accessed through
separate submodules:

- ``hyperspec.json`` (JSON_)
- ``hyperspec.msgpack`` (MessagePack_)
- ``hyperspec.yaml`` (YAML_)
- ``hyperspec.toml`` (TOML_)
...
.. code-block:: python
    ...
    >>> # Encode as msgpack
    ... hyperspec.msgpack.encode({"hello": "world"})
...
.. code-block:: python
    ...
    >>> # Decode msgpack
    ... hyperspec.msgpack.decode(b'\x81\xa5hello\xa5world')
    {'hello': 'world'}
...
.. code-block:: python
    ...
    ... decoder = hyperspec.json.Decoder()
...
``hyperspec`` optionally supports specifying the expected output types during
...
- Often serialized data has a fixed schema (e.g. a request handler in a REST
  ...
  ``hyperspec`` to perform validation during decoding, with *no* added runtime
  cost.

- Python has a much richer type system than serialization protocols like JSON_
  or MessagePack_. Specifying the output types lets ``hyperspec`` decode messages
...
``hyperspec`` uses Python `type annotations`_ to describe the expected types. A
...
Unlike some other libraries (e.g. pydantic_), ``hyperspec`` won't perform any
...
.. code-block:: python
    ...
    >>> hyperspec.json.decode(b'[1, 2, "3"]', type=list[int], strict=False)
    [1, 2, 3]
