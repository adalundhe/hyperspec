...
- Add `hyperspec.inspect.is_struct` and `hyperspec.inspect.is_struct_type` functions for checking whether an object is a `hyperspec.Struct`-like instance or class ({pr}`950`).
- Support Windows `arm64` builds ({pr}`943`).
- Enable ThinLTO on macOS `aarch64` builds ({pr}`937`).
- Fix leaks of `re.Pattern` objects when using `pattern` constraints of `hyperspec.Meta` ({pr}`899`).
- Fixed `memoryview` support for `hyperspec.msgpack` ({pr}`836`).
...
- Automatically infer the input field naming convention when converting
  ...
  `hyperspec.convert` ({pr}`636`).

## Version 0.18.5 (2023-12-12)

- Support unhashable `Annotated` metadata in `hyperspec.inspect.type_info`
  ({pr}`566`).
- Fix bug preventing decoding dataclasses/attrs types with default values and
  `slots=True, frozen=True` ({pr}`569`).
- Support passing parametrized generic struct types to `hyperspec.structs.fields`
  ({pr}`571`).
- Validate `str` constraints on dict keys when decoding msgpack ({pr}`577`).
- Support `UUID` subclasses as inputs to `hyperspec.convert` ({pr}`592`).
...
- Add `hyperspec.structs.force_setattr` ({pr}`600`).
...
- Resolve an issue leading to periodic segfaults when importing `hyperspec` on
  CPython 3.12 ({pr}`561`)
...
- Add support for `attrs` validators ({pr}`538`).
- Relax datetime/time parsing format to allow some RFC3339 extensions from
  ISO8601 ({pr}`539`).

## Version 0.18.1 (2023-08-16)

- Support custom `builtin_types` in `hyperspec.to_builtins` ({pr}`517`).
...
- Add a new `hyperspec.json.Decoder.decode_lines` method for decoding
...
- Add `float_hook` to `hyperspec.json.Decoder` to support changing the default
...
- Add native support for encoding/decoding `datetime.timedelta` types
  ({pr}`475`).
- Add a new `hyperspec.json.Encoder.encode_lines` method for encoding an iterable
  of values as newline-delimited JSON ({pr}`479`).

## Version 0.16.0 (2023-06-12)

- Deprecate `hyperspec.from_builtins` in favor of `hyperspec.convert`. The new
  `convert` function provides a superset of the functionality available in
  the old `from_builtins` function ({pr}`431`).
- Add a `from_attributes` argument to `hyperspec.convert` for allowing
  ...
  ({pr}`419`).
- Support passing generic `Mapping` objects as inputs to `hyperspec.convert`.
  These may be coerced to `dict`/`Struct`/`dataclasses`/`attrs` types
  ({pr}`427`).
- Add a new `strict` keyword argument to all `decode` functions,
  `Decoder` classes, as well as `hyperspec.convert`. This defaults to `True`,
  setting it to false enables a wider set of coercion rules (e.g. coercing a
  `str` input to an `int`). See "Strict" vs "Lax" mode for more information
  ({pr}`434`).
- Support all supported types as inputs to `hyperspec.convert` ({pr}`431`,
  {pr}`418`).
- Passthrough input unchanged when coercing to `typing.Any` type in
  `hyperspec.convert` ({pr}`435`).
...
- Remove JSON compatibility checks when constructing a
  `hyperspec.json.Decoder`. Trying to decode messages into types that JSON
...
- Support encoding and decoding `attrs`
  types ({pr}`323`).
...
- Expose a struct's configuration through a `__struct_config__` attribute
  ({pr}`328`).
- Add `hyperspec.structs.fields` utility function for inspecting the fields
...
- Fix a bug when defining both `lt` and `gt` constraints on an integer
  type ({pr}`335`).
- Fix a bug supporting fields defined with `hyperspec.field()` with no arguments
  ({pr}`343`).
- Allow arbitrary input types to `hyperspec.from_builtins` ({pr}`346`).
- Support decoding into subclasses of `int` & `bytes` in
  `hyperspec.from_builtins` ({pr}`346`).
- Add `hyperspec.UNSET` and `hyperspec.UnsetType` for tracking unset fields. See the
  the docs for more information ({pr}`350`).
- **BREAKING**: In the unlikely event you were using the previous
  `hyperspec.UNSET` singleton to explicitly indicate no default value on struct
  types, you should now make use of `hyperspec.NODEFAULT` instead ({pr}`350`).
...
- Add `from_builtins` function for converting and validating messages composed
  of simple builtin types to ones composed of any type hyperspec supports
  ({pr}`266`, {pr}`302`).
- Add `hyperspec.yaml` module for encoding/decoding YAML ({pr}`267`).
- Add `hyperspec.toml` module for encoding/decoding TOML ({pr}`268`).
- Add `hyperspec.structs.replace` function for creating a copy of an existing
  `Struct` with some changes applied ({pr}`262`).
- Add `hyperspec.structs.asdict` and `hyperspec.structs.astuple` functions for
...
- Allow creating a `hyperspec.Raw` object from a `str` ({pr}`252`).
- Add new experimental `hyperspec.inspect` module for inspecting type
  annotations. This is intended to be used for building downstream tooling
  based on hyperspec-compatible types. See the docs for more
  information ({pr}`253`).
- Add new `extra` field to `hyperspec.Meta`, for storing arbitrary user-defined
  metadata ({pr}`255`).
...
- Add a `tz` `constraint` to require aware or naive
  datetime/time objects when decoding ({pr}`224`).
- Add support for encoding/decoding `datetime.time` objects ({pr}`225`)
- Add a `hyperspec.json.format` utility for efficiently pretty-printing already
...
- Support for `constraints` during validation. For example,
  this allows ensuring a field is an integer >= 0. ({pr}`176`)
- New utilities for generating JSON Schema from type
  definitions ({pr}`181`)
...
- Adds `order` and `eq` configuration options for `Struct` types, mirroring
  ...
  types are very performant, roughly 10x to 70x faster than alternative
  libraries ({pr}`122`).
...
- Add a new `hyperspec.Raw` type for delayed decoding of message fields /
  serializing already encoded fields ({pr}`92`).
- Add `omit_defaults` option to `Struct` types.
  ...
  ({pr}`94`).
- Add `rename` option to `Struct` types for
...
- Improve performance of `nogc=True` structs.
  GC is now avoided in more cases, and `nogc=True` structs use 16 fewer bytes
  per instance. Also added a benchmark for how `hyperspec` can interact with
  application GC usage ({pr}`93`).
- Cache creation of tagged union lookup tables, reducing memory usage for
  applications making heavy use of tagged unions ({pr}`91`).
...
- Support tagged unions for
  encoding/decoding a `Union` of `hyperspec.Struct` types ({pr}`83`).
- Further improve encoding performance of `enum.Enum` instances by 20-30%
  ({pr}`84`).
- Reduce overhead of calling `hyperspec.json.decode`/`hyperspec.msgpack.decode`
...
- Moved MessagePack support to the `hyperspec.msgpack` submodule ({pr}`56`).
- New JSON support available in `hyperspec.json` ({pr}`56`).
- Improved error message generation to provide full path to the mistyped values
  ({pr}`56`).
- Renamed the `immutable` kwarg in `hyperspec.Struct` to `frozen` to better
...
- Adds support for Python 3.10 pattern matching on `hyperspec.Struct` types
  ({pr}`53`).
