...
``hyperspec`` provides type-introspection support, which can be used to build
tooling on top of hyperspec-compatible types. Possible use cases include:

- Generating OpenAPI_ specifications from hyperspec-compatible types (note that
...
The main function here is `hyperspec.inspect.type_info` for converting a type
annotation into a corresponding `hyperspec.inspect.Type` object. There's also
`hyperspec.inspect.multi_type_info` which converts an iterable of annotations;
...
.. code-block:: python
    ...
    >>> hyperspec.inspect.type_info(bool)
    BoolType()

    >>> hyperspec.inspect.type_info(int)
    IntType(gt=None, ge=None, lt=None, le=None, multiple_of=None)

    >>> hyperspec.inspect.type_info(list[int])  # nested types are traversed
    ListType(
    ...
    >>> hyperspec.inspect.multi_type_info([bool, int])  # inspect multiple types
...
.. code-block:: python
    ...
    >>> from hyperspec import Meta

    >>> PositiveInt = Annotated[int, Meta(gt=0)]

    >>> hyperspec.inspect.type_info(PositiveInt)
...
.. code-block:: python
    ...
    >>> hyperspec.inspect.type_info(User)
    StructType(
        cls=User,
        fields=(
            Field(
...
wrapped in a `hyperspec.inspect.Metadata` object. Note that all JSON schema
...
.. code-block:: python
    ...
    >>> hyperspec.inspect.type_info(UnixName)
    Metadata(
        type=StrType(
            min_length=1,
            max_length=32,
...
Every type supported by ``hyperspec`` has a corresponding `hyperspec.inspect.Type`
...
found `here
<https://github.com/jcrist/hyperspec/blob/main/src/hyperspec/_json_schema.py>`__. In
