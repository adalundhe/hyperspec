...
``hyperspec`` includes support for "schema evolution", meaning that:
...
1. Any new fields on a `hyperspec.Struct` must specify default values.
...
For example, suppose we had a `hyperspec.Struct` type representing a user:
...
.. code-block:: python
    ...
    >>> old_dec = hyperspec.json.Decoder(User)

    >>> new_dec = hyperspec.json.Decoder(User2)

    >>> new_msg = hyperspec.json.encode(
