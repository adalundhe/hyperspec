...
<supported-types>`, ``hyperspec`` provides a few callbacks to
``Encoder``/``Decoder``.

- ``enc_hook``, for transforming custom types into values
  that ``hyperspec``:doc:`natively supports <supported-types>`.
...
.. code-block:: python
    ...
    def enc_hook(obj: Any) -> Any:
        """Given an object that hyperspec doesn't know how to serialize by
...
- ``enc_hook`` in ``Encoder``, for transforming custom types into values
  that ``hyperspec`` already knows how to serialize.
...
objects to/from objects, which are then natively handled by ``hyperspec``.
...
.. code-block:: python
    ...
    class MyMessage(hyperspec.Struct):
        field_1: str
        field_2: complex
    ...
    enc = hyperspec.json.Encoder(enc_hook=enc_hook)
    dec = hyperspec.json.Decoder(MyMessage, dec_hook=dec_hook)
...
By default extensions are serialized to/from `hyperspec.msgpack.Ext` objects.

.. code-block:: python

    >>> ext = hyperspec.msgpack.Ext(1, b"some data")  # an extension object, with type code 1
    >>> msg = hyperspec.msgpack.encode(ext)
    >>> ext2 = hyperspec.msgpack.decode(msg)
...
- ``enc_hook`` in `hyperspec.msgpack.Encoder`, for transforming custom types into
  values that ``hyperspec`` already knows how to serialize.

- ``ext_hook`` in `hyperspec.msgpack.Decoder`, for converting extensions back
...
you're communicating with MessagePack libraries other than ``hyperspec``, you'd
...
.. code-block:: python
    ...
    def enc_hook(obj: Any) -> Any:
        if isinstance(obj, complex):
            ...
            # Return an `Ext` object so hyperspec serializes it as an extension type.
            return hyperspec.msgpack.Ext(COMPLEX_TYPE_CODE, data)
        else:
            # Raise a NotImplementedError for other types
            raise NotImplementedError(f"Objects of type {type(obj)} are not supported")
