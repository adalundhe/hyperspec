...
``hyperspec``. They're presented in order from "sane, definitely a good idea" to
"fast, but you may not want to do this".

Reuse Encoders/Decoders
-----------------------

Every call to a top-level ``encode`` function like `hyperspec.json.encode`
...
`hyperspec.json.Encoder`) once and reuse it for all encoding calls, avoiding
...
.. code-block:: python
    ...
    >>> encoder = hyperspec.json.Encoder()  # Create once
...
`hyperspec.json.Decoder`) once and reuse it for each call. Since decoders are
...
.. code-block:: python
    ...
    >>> decoder = hyperspec.json.Decoder(list[int])  # Create once
...
:doc:`structs` are hyperspec's native way of expressing user-defined types.
...
defining a `hyperspec.Struct` type (or types) for your schema and preferring that
...
beneficial to define a smaller `hyperspec.Struct` type that only has the fields
you require.
...
types with only those fields, ``hyperspec`` can avoid doing unnecessary work
...
.. code-block:: python
    ...
    >>> tweet = hyperspec.json.decode(example_json, type=Tweet)
...
the output. ``hyperspec`` exposes an alternative ``Encoder.encode_into`` (e.g.
`hyperspec.json.Encoder.encode_into`) that writes into a pre-allocated
...
.. code-block:: python
    ...
    encoder = hyperspec.msgpack.Encoder()
...
.. code-block:: python
    ...
    json_msg = hyperspec.json.encode(["my", "message"])

    full_payload = json_msg + b'\n'

However, this results in an unnecessary copy of ``json_msg``, which can be
avoided by using `hyperspec.json.Encoder.encode_into`.
...
``hyperspec`` supports both JSON_ and MessagePack_ protocols. The latter is less
...
benefit from using it instead of JSON. And since ``hyperspec`` supports both
protocols with a consistent interface, switching from ``hyperspec.json`` to
``hyperspec.msgpack`` should be fairly painless.
...
to Python's garbage collector (GC). By default, `hyperspec.Struct` types
...
.. code-block:: python
    ...
    >>> class Example(hyperspec.Struct, array_like=True):
    ...     my_first_field: str
    ...     my_second_field: int
    ...
    >>> hyperspec.json.decode(msg, type=Example)
