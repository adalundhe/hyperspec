...
When using :ref:`typed-decoding` ``hyperspec`` will ensure decoded
...
.. code-block:: python
    ...
    >>> hyperspec.json.decode(b"[1, 2, 3]", type=list[int])
    [1, 2, 3]

    >>> hyperspec.json.decode(b'[1, 2, "oops"]', type=list[int])
...
Constraints in ``hyperspec`` are specified by wrapping a type with
`typing.Annotated`, and adding a `hyperspec.Meta` annotation.
...
.. code-block:: python
    ...
    >>> PositiveInt = Annotated[int, hyperspec.Meta(gt=0)]

    >>> hyperspec.json.decode(b'[1, 2, 3]', type=list[PositiveInt])
    [1, 2, 3]

    >>> hyperspec.json.decode(b'[1, 2, -1]', type=list[PositiveInt])
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `int` >= 1 - at `$[2]`
...
.. code-block:: python
    ...
    >>> hyperspec.json.decode(b'-1', type=Annotated[int, hyperspec.Meta(ge=0)])
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `int` >= 0
...
.. code-block:: python
    ...
    ...     b'"invalid username"',
    ...     type=Annotated[str, hyperspec.Meta(pattern="^[a-z0-9_]*$")]
    ... )
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `str` matching regex '^[a-z0-9_]*$'
...
.. code-block:: python
    ...
    ...     type=Annotated[datetime, hyperspec.Meta(tz=True)]  # require timezone aware
    ... )
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `datetime` with a timezone component

    >>> hyperspec.json.decode(
    ...     b'"2022-04-02T18:18:10-06:00"',
    ...     type=Annotated[datetime, hyperspec.Meta(tz=False)]  # require timezone naive
    ... )
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `datetime` with no timezone component
...
.. code-block:: python
    ...
    >>> hyperspec.json.decode(
    ...     b'"ZXhhbXBsZQ=="',
    ...     type=Annotated[bytes, hyperspec.Meta(min_length=10)]
    ... )
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `bytes` of length >= 10
...
.. code-block:: python
    ...
    >>> hyperspec.json.decode(
    ...     b'[1, 2, 3, 4]',
    ...     type=Annotated[list[int], hyperspec.Meta(max_length=3)]
    ... )
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `array` of length <= 3
...
.. code-block:: python
    ...
    >>> hyperspec.json.decode(
    ...     b'{"a": 1, "b": 2, "c": 3, "d": 4}',
    ...     type=Annotated[dict[str, int], hyperspec.Meta(max_length=3)]
    ... )
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `object` of length <= 3
