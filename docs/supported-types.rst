...
``hyperspec`` uses Python `type annotations`_ to describe the expected types.
...
Here we document how hyperspec maps Python objects to/from the various supported
protocols.
...
.. code-block:: python
    ...
    >>> hyperspec.json.encode(None)
    b'null'

    >>> hyperspec.json.decode(b'null')
    None
...
.. code-block:: python
   ...
   >>> hyperspec.json.decode(b'"null"', type=None, strict=False)
   None
...
.. code-block:: python
    ...
    >>> hyperspec.json.encode(True)
    b'true'

    >>> hyperspec.json.decode(b'true')
    True
...
.. code-block:: python
   ...
   >>> hyperspec.json.decode(b'"false"', type=bool, strict=False)
   False

   >>> hyperspec.json.decode(b'"TRUE"', type=bool, strict=False)
   True

   >>> hyperspec.json.decode(b'1', type=bool, strict=False)
   True
...
.. code-block:: python
    ...
    >>> hyperspec.json.encode(123)
    b"123"

    >>> hyperspec.json.decode(b"123", type=int)
    123
...
.. code-block:: python
   ...
   >>> hyperspec.json.decode(b'"123"', type=int, strict=False)
   123

   >>> hyperspec.json.decode(b'123.0', type=int, strict=False)
   123
...
``hyperspec.json`` handles this by encoding these values as ``null``. The
...
.. code-block:: python
    ...
    >>> hyperspec.json.encode(123.0)
    b"123.0"

    >>> # JSON doesn't support nonfinite values, these serialize as null
    ... hyperspec.json.encode(float("nan"))
    b"null"

    >>> hyperspec.json.decode(b"123.0", type=float)
    123.0

    >>> # Ints are automatically converted to floats
    ... hyperspec.json.decode(b"123", type=float)
    123.0
...
.. code-block:: python
   ...
   >>> hyperspec.json.decode(b'"123.45"', type=float, strict=False)
   123.45

   >>> hyperspec.json.decode(b'"-inf"', type=float, strict=False)
   -inf
...
.. code-block:: python
    ...
    >>> hyperspec.json.encode("Hello, world!")
    b'"Hello, world!"'

    >>> hyperspec.json.encode("𝄞 is not escaped")
    b'"\xf0\x9d\x84\x9e is not escaped"'

    >>> hyperspec.json.decode(b'"Hello, world!"')
    "Hello, world!"
...
.. code-block:: python
    ...
    >>> msg = hyperspec.json.encode(b"\xf0\x9d\x84\x9e")

    >>> msg
    b'"85+Eng=="'

    >>> hyperspec.json.decode(msg, type=bytes)
    b'\xf0\x9d\x84\x9e'

    >>> hyperspec.json.decode(msg, type=bytearray)
...
.. code-block:: python
    ...
    >>> msg = hyperspec.json.encode(tz_aware)

    >>> msg
    b'"2021-04-02T18:18:10.000123+06:00"'

    >>> hyperspec.json.decode(msg, type=datetime.datetime)
    ...
    >>> msg = hyperspec.json.encode(tz_naive)
    ...
    >>> hyperspec.json.decode(b'"oops"', type=datetime.datetime)
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Invalid RFC3339 encoded datetime
...
.. code-block:: python
    ...
    >>> hyperspec.json.decode(b"1617405490.000123", type=datetime.datetime)
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `datetime`, got `float`

    >>> hyperspec.json.decode(b"1617405490.000123", type=datetime.datetime, strict=False)
...
.. code-block:: python
    ...
    >>> msg = hyperspec.json.encode(date)

    >>> msg
    b'"2021-04-02"'

    >>> hyperspec.json.decode(msg, type=datetime.date)
    datetime.date(2021, 4, 2)

    >>> hyperspec.json.decode(b'"oops"', type=datetime.date)
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Invalid RFC3339 encoded date
...
.. code-block:: python
    ...
    >>> hyperspec.json.decode(msg, type=datetime.time)
    ...
    >>> hyperspec.json.decode(b'"oops"', type=datetime.time)
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Invalid RFC3339 encoded time
...
While hyperspec will decode duration strings making use of the ``H`` (hour) or
``M`` (minute) units, durations encoded by hyperspec will only consist of ``D``
(day) and ``S`` (second) segments.

The implementation in ``hyperspec`` is compatible with the ones in:
...
Duration strings produced by hyperspec should be interchangeable with these
...
.. code-block:: python
    ...
    >>> hyperspec.json.encode(timedelta(seconds=123))
    b'"PT123S"'

    >>> hyperspec.json.encode(timedelta(days=1, seconds=30, microseconds=123))
    b'"P1DT30.000123S"'

    >>> hyperspec.json.decode(b'"PT123S"', type=timedelta)
    datetime.timedelta(seconds=123)

    >>> hyperspec.json.decode(b'"PT1.5M"', type=timedelta)
    datetime.timedelta(seconds=90)

    >>> hyperspec.json.decode(b'"oops"', type=datetime.timedelta)
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Invalid ISO8601 duration
...
.. code-block:: python
    ...
    >>> hyperspec.json.decode(b"123.4", type=datetime.timedelta)
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `duration`, got `float`

    >>> hyperspec.json.decode(b"123.4", type=datetime.timedelta, strict=False)
...
.. code-block:: python
    ...
    >>> hyperspec.json.encode(u)
    b'"c4524ac0-e81e-4aa8-a595-0aec605a659a"'

    >>> hyperspec.json.decode(b'"c4524ac0-e81e-4aa8-a595-0aec605a659a"', type=uuid.UUID)
    UUID('c4524ac0-e81e-4aa8-a595-0aec605a659a')

    >>> hyperspec.json.decode(b'"oops"', type=uuid.UUID)
    Traceback (most recent call last):
        File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Invalid UUID
...
.. code-block:: python
    ...
    >>> enc = hyperspec.json.Encoder(uuid_format="hex")
    ...
    >>> hyperspec.json.decode(uuid_hex, type=uuid.UUID)
    UUID('c4524ac0-e81e-4aa8-a595-0aec605a659a')

    >>> enc = hyperspec.msgpack.Encoder(uuid_format="bytes")

    >>> uuid_bytes = enc.encode(u)

    >>> hyperspec.msgpack.decode(uuid_bytes, type=uuid.UUID)
    UUID('c4524ac0-e81e-4aa8-a595-0aec605a659a')
...
.. code-block:: python
    ...
    >>> msg = hyperspec.json.encode(x)

    >>> msg
    b'"1.2345"'

    >>> hyperspec.json.decode(msg, type=decimal.Decimal)
    Decimal('1.2345')

    >>> hyperspec.json.decode(b'"oops"', type=decimal.Decimal)
    Traceback (most recent call last):
        File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Invalid decimal string
...
.. code-block:: python
    ...
    >>> encoder = hyperspec.json.Encoder(decimal_format="number")
...
.. code-block:: python
   ...
   >>> hyperspec.json.decode(b"1.3", type=decimal.Decimal)
   Decimal('1.3')

   >>> hyperspec.json.decode(b"1.300", type=decimal.Decimal)
   Decimal('1.300')

   >>> hyperspec.json.decode(b"0.1234567891234567811", type=decimal.Decimal)
...
.. code-block:: python
   ...
   >>> hyperspec.yaml.decode(b"1.3", type=decimal.Decimal)
   Decimal('1.3')

   >>> hyperspec.yaml.decode(b"1.300", type=decimal.Decimal)  # trailing 0s truncated!
   Decimal('1.3')

   >>> hyperspec.yaml.decode(b"0.1234567891234567811", type=decimal.Decimal)  # precision loss!
...
.. code-block:: python
    ...
    >>> hyperspec.json.encode([1, 2, 3])
    b'[1,2,3]'

    >>> hyperspec.json.encode({1, 2, 3})
    b'[1,2,3]'

    >>> hyperspec.json.decode(b'[1,2,3]', type=set)
    {1, 2, 3}

    >>> from typing import Set

    >>> # Decode as a set of ints
    ... hyperspec.json.decode(b'[1, 2, 3]', type=Set[int])
    {1, 2, 3}

    >>> # Oops, all elements should be ints
    ... hyperspec.json.decode(b'[1, 2, "oops"]', type=Set[int])
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `int`, got `str` - at `$[2]`
...
Note that ``hyperspec`` supports both `typing.NamedTuple` and
`collections.namedtuple`, although the latter lacks a way to specify field
types.

When possible we recommend using `hyperspec.Struct` (possibly with
...
.. code-block:: python
    ...
    >>> msg = hyperspec.json.encode(ben)

    >>> hyperspec.json.decode(msg, type=Person)
    Person(name='ben', age=25)

    >>> wrong_type = b'["chad", "twenty"]'

    >>> hyperspec.json.decode(wrong_type, type=Person)
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `int`, got `str` - at `$[1]`
...
.. code-block:: python
    ...
    >>> hyperspec.json.encode({"x": 1, "y": 2})
    b'{"x":1,"y":2}'

    >>> from typing import Dict

    >>> # Decode as a Dict of str -> int
    ... hyperspec.json.decode(b'{"x":1,"y":2}', type=Dict[str, int])
    {"x": 1, "y": 2}

    >>> # Oops, there's a mistyped value
    ... hyperspec.json.decode(b'{"x":1,"y":"oops"}', type=Dict[str, int])
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `int`, got `str` - at `$[...]`
...
information during decoding. Note that ``hyperspec`` supports both
...
When possible we recommend using `hyperspec.Struct` instead of ``TypedDict`` for
specifying schemas - :doc:`structs` are faster, more ergonomic, and support
...
.. code-block:: python
    ...
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `int`, got `str` - at `$.age`
...
When possible we recommend using `hyperspec.Struct` instead of dataclasses for
specifying schemas - :doc:`structs` are faster, more ergonomic, and support
additional features.
...
.. code-block:: python
    ...
    >>> msg = hyperspec.json.encode(carol)
...
will be called, and a `hyperspec.ValidationError` raised on error. Note that
attrs' `converters
<https://www.attrs.org/en/stable/examples.html#conversion>`__ are not currently
supported.

When possible we recommend using `hyperspec.Struct` instead of attrs_ types for
specifying schemas - :doc:`structs` are faster, more ergonomic, and support
additional features.
...
Structs are the preferred way of defining structured data types in ``hyperspec``.
...
By default `hyperspec.Struct` types map to objects/maps in all protocols. During
...
.. code-block:: python
    ...
    >>> hyperspec.json.decode(wrong_type, type=User)
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `str`, got `int` - at `$.groups[1]`
...
.. code-block:: python
    ...
    >>> class User(hyperspec.Struct, array_like=True):
    ...     name: str
    ...     groups: Set[str] = set()
    ...     email: Optional[str] = None
    ...
    >>> hyperspec.json.decode(b'["bob"]', type=User)
    User(name="bob", groups=[], email=None)

    >>> hyperspec.json.decode(b'["carol", ["admin"], null, ["extra", "field"]]', type=User)
    User(name="carol", groups=["admin"], email=None)

    >>> hyperspec.json.decode(b'["david", ["finance", 123]]')
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `str`, got `int` - at `$[1][1]`
...
`hyperspec.UNSET` is a singleton object used to indicate that a field has no set
...
``UNSET`` fields are supported for `hyperspec.Struct`, `dataclasses`, and attrs_
types. It is an error to use `hyperspec.UNSET` or `hyperspec.UnsetType` anywhere
...
.. code-block:: python
    ...
    >>> hyperspec.json.encode(Fruit.APPLE)
    b'"apple"'

    >>> hyperspec.json.decode(b'"apple"', type=Fruit)
    <Fruit.APPLE: 'apple'>

    >>> hyperspec.json.decode(b'"grape"', type=Fruit)
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Invalid enum value 'grape'
    ...
    >>> hyperspec.json.encode(JobState.RUNNING)
    b'1'

    >>> hyperspec.json.decode(b'2', type=JobState)
    <JobState.SUCCEEDED: 2>

    >>> hyperspec.json.decode(b'4', type=JobState)
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Invalid enum value 4
...
.. code-block:: python
    ...
    >>> hyperspec.json.decode(b'1', type=Literal[1, 2, 3])
    1

    >>> hyperspec.json.decode(b'"one"', type=Literal["one", "two", "three"])
    'one'

    >>> hyperspec.json.decode(b'4', type=Literal[1, 2, 3])
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Invalid enum value 4

    >>> hyperspec.json.decode(b'"bad"', type=Literal[1, 2, 3])
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `int`, got `str`
...
.. code-block:: python
    ...
    >>> hyperspec.json.encode(UserId(1234))
    b'1234'

    >>> hyperspec.json.decode(b'1234', type=UserId)
    1234

    >>> hyperspec.json.decode(b'"oops"', type=UserId)
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `int`, got `str`
...
Here ``Point`` is a "type alias" for ``tuple[float, float]`` - ``hyperspec``
will substitute in ``tuple[float, float]`` whenever the ``Point`` type
is used in an annotation.

``hyperspec`` supports the following equivalent forms:
...
``hyperspec`` supports generic types, including `user-defined generic types`_
...
.. code-block:: python
    ...
    class Paginated(hyperspec.Struct, Generic[T]):
...
.. code-block:: python
    ...
    msg = hyperspec.json.decode(json_str, type=Paginated[User])
    print(msg)
    #> Paginated(
...
.. code-block:: python
    ...
    hyperspec.json.decode(some_other_message, type=Paginated[Team])
...
.. code-block:: python
    ...
    hyperspec.json.decode(some_other_message, type=Paginated)
    hyperspec.json.decode(some_other_message, type=Paginated[Any])
...
.. code-block:: python
    ...
    class Example(hyperspec.Struct, Generic[S]):
        value: S
    ...
    hyperspec.json.decode(some_other_message, type=Example)
    hyperspec.json.decode(some_other_message, type=Example[Sequence])
...
``hyperspec`` supports several "abstract" types, decoding them as
...
.. code-block:: python
    ...
    >>> hyperspec.json.decode(b'{"x": 1}', type=MutableMapping[str, int])
    {"x": 1}

    >>> hyperspec.json.decode(b'{"x": "oops"}', type=MutableMapping[str, int])
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `int`, got `str` - at `$[...]`
...
.. code-block:: python
    ...
    ... decoder = hyperspec.json.Decoder(Union[int, str, List[str]])
    ...
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
    hyperspec.ValidationError: Expected `int | str | array`, got `bool`

``Raw``
-------

`hyperspec.Raw` is a buffer-like type containing an already-encoded message.
They have two common uses:

**1. Avoiding unnecessary encoding cost**

Wrapping an already-encoded buffer in `hyperspec.Raw` lets the encoder avoid
...
.. code-block:: python
    ...
    ... fragment = hyperspec.Raw(b'{"x": 1, "y": 2}')
...
in a message. ``hyperspec`` provides an optimized version of one common pattern
(:ref:`struct-tagged-unions`), but if you need to do something more complicated
you may find using `hyperspec.Raw` useful here.
...
.. code-block:: python
    ...
    >>> class Point1D(hyperspec.Struct):
    ...     x: int

    >>> class Point2D(hyperspec.Struct):
    ...     x: int
    ...     y: int

    >>> class Point3D(hyperspec.Struct):
    ...     x: int
    ...     y: int
    ...     z: int

    >>> class Model(hyperspec.Struct):
    ...     dimensions: int
    ...     point: hyperspec.Raw  # use hyperspec.Raw to delay decoding the point field
    ...
    ...     # on `point`.
    ...     model = hyperspec.json.decode(msg, type=Model)
    ...
    ...
    ...     return hyperspec.json.decode(model.point, type=point_type)
...
- ``map``: `dict`
- ``ext``: `hyperspec.msgpack.Ext`, `datetime.datetime`, or a custom type
...
.. _open an issue: https://github.com/jcrist/hyperspec/issues>
