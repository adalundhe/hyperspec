...
``hyperspec`` provides builtin support for several common protocols (``json``,
``msgpack``, ``yaml``, and ``toml``). Support for additional protocols may be
added by combining a serialization library with hyperspec's *converter
functions*: `hyperspec.to_builtins` and `hyperspec.convert`.

- `hyperspec.to_builtins`: takes an object composed of any :doc:`supported type
...
- `hyperspec.convert`: takes an object composed of any :doc:`supported type
...
For example, if ``hyperspec`` didn't already provide support for ``json``, you
...
.. code-block:: ipython
    ...
    In [2]: def encode(obj):
       ...:     return json.dumps(hyperspec.to_builtins(obj))

    In [3]: def decode(msg, type=Any):
       ...:     return hyperspec.convert(json.loads(msg), type=type)

    In [4]: class Point(hyperspec.Struct):
       ...:     x: int
       ...:     y: int
    ...
    ---> 2     return hyperspec.convert(json.loads(msg), type=type)
...
- ``from_attributes``: `convert` only. If True, input objects may be coerced
  ...
  query results (ORM or otherwise) to hyperspec structured types. The default is
  False.

- ``enc_hook``/``dec_hook``: the standard keyword arguments used for
  :doc:`extending` hyperspec to support additional types.
...
If ``hyperspec`` didn't already provide support for ``toml``, you could add
...
``hyperspec`` uses these APIs to implement ``toml`` and ``yaml`` support,
wrapping external serialization libraries:

- ``hyperspec.toml`` (`code <https://github.com/jcrist/hyperspec/blob/main/src/hyperspec/toml.py>`__)

- ``hyperspec.yaml`` (`code <https://github.com/jcrist/hyperspec/blob/main/src/hyperspec/yaml.py>`__)

The implementation in ``hyperspec.toml`` is *almost* identical to the one above,
